...
<div class="page-body">
    <div class="container-xl">
        <div class="card">
            ...
            <div class="table-responsive"><table class="table table-vcenter card-table"><thead><tr><th>Code</th><th>Name</th><th>Status</th><th class="w-1"></th></tr></thead><tbody>@forelse($records as $record)<tr><td><code>{{ $record->subject_code }}</code></td><td>{{ $record->subject_name }}</td><td><span class="badge {{ $record->is_active ? 'bg-success-lt' : 'bg-secondary-lt' }}">{{ $record->is_active ? 'Active' : 'Inactive' }}</span></td><td><a class="btn btn-sm btn-outline-primary" href="{{ route($routePrefix.'.edit', $record) }}">Edit</a></td></tr>@empty<tr><td colspan="4" class="text-center text-secondary py-4">No records found.</td></tr>@endforelse</tbody></table></div>
