                    ...
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12">
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                <button type="button" class="btn btn-outline-secondary" id="dq-reset-builder">Reset Builder</button>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function readJsonResponse(res, fallback='Request failed.') {
        ...
        try{return JSON.parse(text);}catch(_e){
            ...
            try{
                ...
                detail=(doc.querySelector('h1')?.textContent||doc.querySelector('title')?.textContent||'').trim();
            }catch(_ignored){}
            const suffix=detail?` · ${detail}`:'';
            throw new Error(`${fallback} Server returned a non-JSON response (HTTP ${res.status})${suffix}.`);
        }
    }
    const opLabel=o=>({eq:'Equals',neq:'Not equal',contains:'Contains',starts_with:'Starts with',ends_with:'Ends with',contains_choice:'Contains choice code',not_contains_choice:'Does not contain choice code',contains_any:'Contains any of',contains_all:'Contains all of',gt:'Greater than',gte:'Greater/equal',lt:'Less than',lte:'Less/equal',between:'Between',in:'In list',is_blank:'Is blank',is_not_blank:'Is not blank'})[o]||o;
    const typeLabel=t=>String(t||'string').replace(/[-_]+/g,' ').toUpperCase();
    ...
    async function refreshHistory(){const url=new URL(root.dataset.historyUrl,window.location.origin);if(currentSavedReportId)url.searchParams.set('saved_report_id',currentSavedReportId);const res=await fetch(url,{headers:{Accept:'application/json'}});if(res.ok){recentRuns=(await readJsonResponse(res,'Unable to refresh execution history.')).runs||[];renderHistory();}}
    ...
    function valueHtml(f,op,value=null,value2=null){
        ...
        const choiceList=f.type==='choice-list';
        const placeholder=choiceList
            ? (['contains_any','contains_all'].includes(op)?'Choice codes, comma-separated':'Exact choice code')
            : (op==='in'?'Comma-separated values':'Value');
        return `<input type="${inputType}"${step} class="form-control form-control-sm dq-c-value" value="${esc(Array.isArray(value)?value.join(', '):(value??''))}" placeholder="${placeholder}">${choiceList?'<div class="form-hint">Choice codes are matched as exact list items, never as substrings.</div>':''}`;
    }
    ...
    function addAggregate(initial={}){ const row=document.createElement('div');row.className='row g-1 dq-aggregate';row.innerHTML=`<div class="col-4"><select class="form-select form-select-sm dq-a-field"><option value="">Field…</option>${fieldOptions(f=>f.aggregates?.length)}</select></div><div class="col-2"><select class="form-select form-select-sm dq-a-function" disabled><option value="">Function</option></select></div><div class="col-2"><select class="form-select form-select-sm dq-a-sort"><option value="">Order: None</option><option value="asc">Ascending</option><option value="desc">Descending</option></select></div><div class="col-3"><input class="form-control form-control-sm dq-a-label" placeholder="Column title" value="${esc(initial.label||'')}"></div><div class="col-1"><button class="btn btn-sm btn-outline-danger dq-remove" type="button">×</button></div>`;const f=row.querySelector('.dq-a-field'),fn=row.querySelector('.dq-a-function'),sort=row.querySelector('.dq-a-sort');f.onchange=()=>{const m=byId[f.value];fn.disabled=!m;fn.innerHTML='<option value="">Function</option>'+(m?m.aggregates.map(a=>`<option value="${a}">${a.replace('_',' ').toUpperCase()}</option>`).join(''):'');};if(initial.field){f.value=initial.field;f.onchange();fn.value=initial.function||'';}sort.value=initial.sort_direction||'';row.querySelector('.dq-remove').onclick=()=>row.remove();document.getElementById('dq-aggregates').appendChild(row); }
    function definition(){ const labels={...labelOverrides};document.querySelectorAll('.dq-label').forEach(e=>{labels[e.dataset.id]=e.value;labelOverrides[e.dataset.id]=e.value;}); const summary=document.getElementById('dq-mode').value==='summary'; return {mode:summary?'summary':'detail',fields:[...selected],conditions:readGroup(document.querySelector('#dq-conditions > .dq-condition-group')),sorts:[...document.querySelectorAll('.dq-sort')].map(r=>({field:r.querySelector('.dq-s-field').value,direction:r.querySelector('.dq-s-dir').value})).filter(x=>x.field),groups:summary?[...document.querySelectorAll('.dq-group-select')].map(e=>e.value).filter(Boolean):[],aggregates:summary?[...document.querySelectorAll('.dq-aggregate')].map(r=>({field:r.querySelector('.dq-a-field').value,function:r.querySelector('.dq-a-function').value,label:r.querySelector('.dq-a-label').value,sort_direction:r.querySelector('.dq-a-sort').value})).filter(x=>x.field&&x.function):[],labels,preview_size:Number(document.getElementById('dq-preview-size').value),report_title:document.getElementById('dq-title').value,show_serial:document.getElementById('dq-show-serial').checked,show_page_number:document.getElementById('dq-show-page').checked,show_timestamp:document.getElementById('dq-show-time').checked}; }
    ...
    document.getElementById('dq-preview').onclick=async()=>{const b=document.getElementById('dq-preview'),msg=document.getElementById('dq-message'),table=document.getElementById('dq-preview-table');b.disabled=true;msg.className='text-secondary';msg.textContent='Running controlled preview…';table.innerHTML='';try{const def=definition(),res=await fetch(root.dataset.previewUrl,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':csrf()},body:JSON.stringify({...def,saved_report_id:currentSavedReportId})}),data=await readJsonResponse(res,'Preview failed.');if(!res.ok){const detail=Object.values(data.errors||{}).flat().join(' ')||data.message||'Preview failed.';const code=data.database_error_code?` [DB: ${data.database_error_code}${data.driver_error_code?'/'+data.driver_error_code:''}]`:'';const ref=data.error_reference?` [Reference: ${data.error_reference}]`:'';throw new Error(detail+code+ref);}document.getElementById('dq-count').textContent=`${Number(data.count).toLocaleString()} matching`;msg.textContent=(data.warnings?.length?data.warnings.join(' '):'Preview uses current authoritative module data.')+` · ${data.duration_ms??0} ms`;const footer=(data.totals&&Object.keys(data.totals).length&&data.total_label)?`<tfoot><tr class="fw-bold bg-light">${def.show_serial?'<td></td>':''}${(()=>{let placed=false;return data.columns.map(c=>{if(Object.prototype.hasOwnProperty.call(data.totals,c.id))return `<td>${esc(data.totals[c.id]??'—')}</td>`;if(!placed){placed=true;return `<td>${esc(data.total_label)}</td>`;}return '<td></td>';}).join('');})()}</tr></tfoot>`:'';table.innerHTML=`<table class="table table-vcenter table-sm"><thead><tr>${def.show_serial?'<th>#</th>':''}${data.columns.map(c=>`<th>${esc(c.label)}</th>`).join('')}</tr></thead><tbody>${data.rows.map((r,i)=>`<tr>${def.show_serial?`<td>${i+1}</td>`:''}${data.columns.map(c=>`<td>${esc(r[c.id]??'—')}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${data.columns.length+(def.show_serial?1:0)}" class="text-secondary text-center">No matching records.</td></tr>`}</tbody>${footer}</table>`;if(currentSavedReportId)await refreshHistory();}catch(e){msg.className='text-danger';msg.textContent=e.message;}finally{b.disabled=false;}};
