...
<div class="card mb-3"><div class="table-responsive"><table class="table table-vcenter"><thead><tr><th>Candidate</th><th>Track</th><th>Grand Total (G/T)</th><th>Common</th><th>General</th><th>Technical</th><th>all_merit_tech</th><th>Status</th><th></th></tr></thead><tbody>@forelse($rows as $r)<tr><td><strong>{{ $r->reg }}</strong><br><span>{{ $r->candidate_name ?: '—' }}</span><br><span class="text-secondary small">{{ $r->user_id }}</span></td><td><span class="badge {{ $trackBadge((string)$r->written_qualified_track) }}">{{ strtoupper((string)$r->written_qualified_track) }}</span></td><td class="text-nowrap"><div><span class="text-secondary small">General</span> <strong>{{ $r->general_grand_total !== null ? number_format((float) $r->general_grand_total, 2) : '—' }}</strong></div><div><span class="text-secondary small">Technical</span> <strong>{{ $r->technical_grand_total !== null ? number_format((float) $r->technical_grand_total, 2) : '—' }}</strong></div></td><td>{{ $r->common_merit_position ?? '—' }}</td><td>{{ $r->general_merit_position ?? '—' }}</td><td>{{ $r->technical_merit_position ?? '—' }}</td><td><code>{{ \App\Models\MeritResult::allMeritTechJson($r->all_merit_tech) }}</code></td><td>{{ $r->status_reason ?? 'MERIT_RANKED' }}</td><td>@if(in_array($state->status,['review_ready','finalized'],true)&&!$state->is_stale&&$state->latest_run_id===$r->processing_run_id&&$run->status==='completed')<a class="btn btn-sm btn-outline-primary" href="{{ route('merit.show',$r) }}">{{ $state->status==='finalized' ? 'Finalized View' : 'Review View' }}</a>@endif</td></tr>@empty<tr><td colspan="9" class="text-center text-secondary py-4">No Merit rows match the selected filters.</td></tr>@endforelse</tbody></table></div><div class="card-footer d-flex justify-content-between align-items-center"><div class="text-secondary">Displaying {{ $rows->firstItem() ?? 0 }} to {{ $rows->lastItem() ?? 0 }} of {{ number_format($rows->total()) }} records</div><div>{{ $rows->links() }}</div></div></div>
