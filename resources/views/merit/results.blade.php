...
<div class="card mb-3">
    <div class="card-header d-flex align-items-center">
        <div>
            <h3 class="card-title">Merit Tie Review</h3>
            <div class="card-subtitle">Non-blocking Commission review after Grand Total → Written → Preliminary → Older DOB. Applies only to Common, General and Technical Merit.</div>
        </div>
        @if(in_array($state->status, ['review_ready', 'finalized'], true) && !$state->is_stale)
            <div class="ms-auto d-flex gap-2">
                <a class="btn btn-sm btn-outline-danger" href="{{ route('merit.tie-review.pdf') }}">Export PDF</a>
                <a class="btn btn-sm btn-outline-success" href="{{ route('merit.tie-review.xlsx') }}">Export XLSX</a>
            </div>
        @endif
    </div>
    <div class="card-body">
        <div class="row g-3 mb-3">
            @foreach(['common' => 'Common Merit', 'general' => 'General Merit', 'technical' => 'Technical Merit'] as $key => $label)
                <div class="col-md-4">
                    <div class="text-secondary">{{ $label }}</div>
                    <div class="h3 mb-0">{{ number_format($tieReview['summary'][$key]['groups']) }} tie groups</div>
                    <div class="small text-secondary">{{ number_format($tieReview['summary'][$key]['candidates']) }} candidates</div>
                </div>
            @endforeach
        </div>

        @if(empty($tieReview['groups']))
            <div class="alert alert-success mb-0">No unresolved business-rule tie found.</div>
        @else
            <div class="table-responsive">
                <table class="table table-sm table-vcenter">
                    ...
                    <tbody>
                        @foreach($tieReview['groups'] as $group)
                            @foreach($group['candidates'] as $candidate)
                                <tr>
                                    <td>{{ $group['scope_label'] }} #{{ $group['group_no'] }}</td>
                                    <td>
                                        <strong>{{ $candidate['reg'] }}</strong><br>
                                        <span class="small">{{ $candidate['name'] ?: '—' }}</span>
                                    </td>
                                    <td>{{ number_format((float) $candidate['grand_total'], 2) }}</td>
                                    <td>{{ number_format((float) $candidate['written_total'], 2) }}</td>
                                    <td>{{ $candidate['preliminary_mark'] }}</td>
                                    <td>{{ $candidate['dob'] }}</td>
                                    <td>{{ $candidate['graduation_year'] ?? '—' }}</td>
                                    <td><strong>{{ $candidate['final_merit_position'] }}</strong></td>
                                </tr>
                            @endforeach
                        @endforeach
                    </tbody>
...
<div class="card mb-3"><div class="card-header"><div><h3 class="card-title">Technical Cadre-wise Merit Lists</h3><div class="card-subtitle">Eligibility-based Technical cadre merit; candidate choice is not used in Merit Generation.</div></div></div><div class="card-body d-flex flex-wrap gap-2">@forelse($cadres as $c)<a class="btn btn-sm btn-outline-primary" href="{{ route('merit.cadre',['cadreCode'=>$c->cadre_code,'run'=>$run->id]) }}">{{ $c->cadre_code }} ({{ $c->cadre_abbr }}) - {{ number_format($c->candidate_count) }}</a>@empty<span class="text-secondary">No cadre-wise merit list generated.</span>@endforelse</div></div>
