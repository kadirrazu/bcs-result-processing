...
<div class="card mb-3">
    ...
    <div class="card-body"><div class="row g-3">
        ...
        <div class="col-md-3"><label class="form-label">District</label><select id="district-code" name="district_code" class="form-select"><option value="">Select</option>@foreach($districts as $item)<option value="{{ $item->code }}" data-division="{{ $item->division_code }}" @selected((string) old('district_code', $record?->district_code) === (string) $item->code)>{{ $item->code }} - {{ $item->name }}</option>@endforeach</select></div>
        @php
