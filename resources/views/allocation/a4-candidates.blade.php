...
@section('content')
<style>
    .a4-results-table thead th,.a4-movement-table thead th{font-weight:700}
    .a4-results-table th,.a4-results-table td,.a4-movement-table th,.a4-movement-table td{text-align:center;vertical-align:middle}
.a4-results-table td.a4-reg-cell{text-align:left!important}
</style>
<div class="page-header"><div class="container-xl"><div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
    <div><h2 class="page-title">A4 Candidate Results · Run v{{ $a4Run->version }}</h2><div class="text-secondary">Candidate-level A4 allocation review and movement evidence.</div></div>
    <div class="d-flex gap-2 flex-wrap"><a class="btn btn-outline-primary" href="{{ route('allocation.a4.show',$a4Run) }}">A4 Seat Ledger</a><a class="btn btn-outline-secondary" href="{{ route('allocation.index') }}">Back to Allocation</a></div>
</div></div></div>
...
<div class="card mb-3">
    <div class="card-header"><div><h3 class="card-title">A4 Candidate Results</h3><div class="card-subtitle">All A4 selections are target-merit based. Retained A3 quota basis remains quota until released or normalized.</div></div></div>
    <div class="card-body border-bottom"><form method="GET" class="row g-2 align-items-end">
        ...
        <div class="col-md-2"><button class="btn btn-primary">Filter</button> <a class="btn btn-outline-secondary" href="{{ route('allocation.a4.candidates',$a4Run) }}">Reset</a></div>
    </form><div class="small text-secondary mt-3">Filtered results: {{ number_format($results->total()) }}</div></div>
    <div class="table-responsive"><table class="table table-vcenter mb-0 a4-results-table"><thead><tr><th>SL</th><th>Reg</th><th>Cadre Code</th><th>Cadre Abbreviation</th><th>Choice</th><th>Merit</th><th>Basis</th><th>Movement</th><th>Original Cadre</th><th>Original Choice</th><th>Reason</th></tr></thead><tbody>
    @forelse($results as $row)<tr>
        <td>{{ $results->firstItem()+$loop->index }}</td><td class="a4-reg-cell"><strong>{{ $row->reg }}</strong></td><td><strong>{{ $row->cadre_code }}</strong></td><td><strong>{{ $abbreviationByCode->get((int)$row->cadre_code,'—') }}</strong></td>
