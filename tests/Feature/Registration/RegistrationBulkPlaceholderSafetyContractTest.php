...
final class RegistrationBulkPlaceholderSafetyContractTest extends TestCase
{
    public function test_registration_staging_and_merge_are_capped_below_mysql_placeholder_limit(): void
    {
        ...
        $this->assertStringContainsString('$stagingColumnCount = count($this->toStagingRow(', $import);
        $this->assertStringContainsString('$this->safeBulkWriteSize($requestedChunkSize, $stagingColumnCount)', $import);
        $this->assertStringContainsString("bulk_placeholder_budget', 60000", $import);

        $this->assertStringContainsString('$this->registrationMergeColumnCount()', $approval);
        $this->assertStringContainsString('return 32;', $approval);
        $this->assertStringContainsString("bulk_placeholder_budget', 60000", $approval);
    }
}
