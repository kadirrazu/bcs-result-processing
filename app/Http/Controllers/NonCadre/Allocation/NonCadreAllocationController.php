...
{
    public function index(NonCadreAllocationService $s):View{$prerequisites=$s->prerequisites();$runs=$s->runs();$latest=$runs->first();return view('non-cadre.allocation.index',['prerequisites'=>$prerequisites,'runs'=>$runs,'statusBoard'=>$s->statusBoard($latest,$prerequisites),'population'=>$latest?$s->populationBreakdown((int)$latest->id):null]);}
    ...
    public function run(int $run,Request $request,NonCadreAllocationService $s):View{$filters=$request->only(['search','post_code','basis','movement','allocation']);return view('non-cadre.allocation.run',['run'=>$s->run($run),'results'=>$s->results($run,$filters),'checks'=>$s->checks($run),'reviews'=>$s->reviews($run),'summary'=>$s->allocationSummary($run),'postSummary'=>$s->postWiseSummary($run),'population'=>$s->populationBreakdown($run),'filters'=>$filters]);}
