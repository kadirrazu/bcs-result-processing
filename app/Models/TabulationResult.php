...
{
    protected $fillable=['processing_run_id','processing_version','registration_id','preliminary_result_id','written_result_id','viva_result_id','user_id','reg','cadre_category','birth_date','graduation_year','written_qualified_track','preliminary_mark','general_written_total','technical_written_total','viva_mark','general_grand_total','technical_grand_total','general_pf','technical_pf','general_merit_eligible','technical_merit_eligible','validation_status','validation_errors','review_warnings','source_snapshot','processing_flags','processed_at'];
    protected function casts():array{return['cadre_category'=>'integer','birth_date'=>'date','graduation_year'=>'integer','preliminary_mark'=>'decimal:2','general_written_total'=>'decimal:2','technical_written_total'=>'decimal:2','viva_mark'=>'decimal:2','general_grand_total'=>'decimal:2','technical_grand_total'=>'decimal:2','general_merit_eligible'=>'boolean','technical_merit_eligible'=>'boolean','validation_errors'=>'array','review_warnings'=>'array','source_snapshot'=>'array','processing_flags'=>'array','processed_at'=>'datetime'];}
    ...
    public static function grandTotalDisplayFor(?string $track,string $side,mixed $value):string
    {
        ...
        if($side==='general'){
            if($track==='T')return 'TRACK_FAILED';
            if($track==='TT')return 'NOT_APPLICABLE';
            return $value===null?'—':number_format((float)$value,2,'.','');
        }
        if($track==='GN')return 'TRACK_FAILED';
        if($track==='GG')return 'NOT_APPLICABLE';
        return $value===null?'—':number_format((float)$value,2,'.','');
    }
