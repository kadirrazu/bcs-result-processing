...
use App\Services\ChoiceOptimization\ChoiceOptimizationHistoricalPullService;
...
final class ProcessChoiceOptimizationHistoricalPull implements ShouldQueue
{
    ...
    public function handle(
        ...
        ChoiceOptimizationHistoricalPullService $service,
    ): void {
        $exam = Examination::query()->findOrFail($this->examinationId);
        $connections->configure($exam);

        try {
            $service->pull($this->sourceId, $this->actorId);
        } finally {
