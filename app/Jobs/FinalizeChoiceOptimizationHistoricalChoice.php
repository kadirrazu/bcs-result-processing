...
final class FinalizeChoiceOptimizationHistoricalChoice implements ShouldQueue
{
    ...
    public function handle(
        ...
        ChoiceOptimizationHistoricalChoiceFinalizationService $service,
    ): void {
        $exam = Examination::query()->findOrFail($this->examinationId);
        $connections->configure($exam);

        try {
            $service->finalize($this->actorId);
        } finally {
