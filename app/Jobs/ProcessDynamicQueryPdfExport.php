...
use App\Reports\Pdf\DynamicQueryPdfReport;
...
final class ProcessDynamicQueryPdfExport implements ShouldQueue
{
    ...
    public function handle(
        ...
        try {
            $run = ReportingExportRun::query()->where('module', 'dynamic_query')->where('export_type', 'PDF')->findOrFail($this->exportRunId);
            ...
            if ($definition === []) {
                throw new RuntimeException('Dynamic report definition is missing.');
            }

            ReportingExportRun::query()->whereKey($run->id)->update([
                'status' => 'running', 'phase' => 'PREPARING', 'progress_percent' => 5,
                'progress_message' => 'Preparing semantic query and current authoritative sources.', 'started_at' => now(),
            ]);
            ...
            ReportingExportRun::query()->whereKey($run->id)->update([
                'phase' => 'RENDERING_PDF', 'progress_percent' => 15,
                'progress_total' => $total, 'progress_message' => 'Authoritative dataset prepared. Rendering PDF.',
            ]);

            $progress = function (int $current, int $ignored) use ($run, $total): void {
                $percent = $total > 0 ? 15 + (int) floor(min(1, $current / $total) * 75) : 90;
                ReportingExportRun::query()->whereKey($run->id)->update([
                    'phase' => 'RENDERING_PDF', 'progress_percent' => min(90, $percent),
                    'progress_current' => $current, 'progress_total' => $total,
                    'progress_message' => 'Rendering report rows to PDF.',
                ]);
            };

            $path = $files->outputPath('dynamic-query', (int) $run->id, 'pdf');
            $generated = $pdf->generateToFile(
                $path,
                $definition,
                $dataset['columns'],
                $dataset['rows'],
            ...
            ReportingExportRun::query()->whereKey($run->id)->update([
                ...
                'progress_current' => (int) ($generated['processed'] ?? $total), 'progress_total' => $total,
                'progress_message' => 'PDF report completed and is ready to download.',
                'file_path' => $path, 'file_name' => $generated['filename'], 'file_mime' => 'application/pdf',
                'file_hash' => hash_file('sha256', $path) ?: null, 'completed_at' => now(), 'failure_message' => null,
            ]);
            ...
            ReportingExportRun::query()->whereKey($this->exportRunId)->update([
                ...
                'progress_message' => 'Dynamic Query PDF generation failed.',
                'failure_message' => mb_substr($e->getMessage(), 0, 65000), 'completed_at' => now(),
            ]);
